...
var monthPicker = {
    ...
    create: function(id, conf) {
        ...
        if (that._conf.period && that._conf.start_month != '' && that._conf.end_month != '') {
            ...
            that._conf.autoCommit && that._conf.callback({
                startDate: that._conf.start_month + '-01',
                endDate: that._conf.end_month + '-31'
            });

        } else {
            this._conf.lastMonth && (this.assemble.month = new Date().getMonth());
            '' !== this._conf.defaultMonth && (this.assemble = {
                'year': this._conf.defaultMonth.substr(0, 4),
                'month': this._conf.defaultMonth.substr(4) * 1
            });
            //默认填充最近一个月
    ...
    periodCss: function() {
        var start = that.util.format({
            year: monthPicker.Params.start_year,
            month: monthPicker.Params.start_month
        });
        var end = that.util.format({
            year: monthPicker.Params.end_year,
            month: monthPicker.Params.end_month
        });
